...
def api_root(request, format=None):
    return Response(
        {
            "ultimate_frisbee_management": "http://"
            + request.get_host()
            + "/api/ultimate_frisbee_management/",  # dont use namespaces with rest framework
...
class managedClubViewset(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        person = pm.Club.objects.filter(
            ~Q(persontoclubmembership__role="member"), persontoclubmembership__person__user=user
        )
